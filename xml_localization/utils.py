...
def traverse_structure(structure, callback):
  if not isinstance(structure['vector']['container'], list):
    structure['vector']['container'] = [structure['vector']['container']]
...
def parse_structure(structure, error_callback):
  ...
  def traverse_structure_callback(container_obj, container_name):
    ...
    extracted_strings += traverse_strings(
      container_obj,
      container_name,
      traverse_strings_callback
    )

  traverse_structure(
    structure,
    traverse_structure_callback
  )
...
def update_structure(structure, updated_strings):
  ...
  def update_structure_callback(container_obj, container_name):
      ...
      for language_code, string in line.items():
        ...
        structure_obj.append({
          **{
            '@'+key:value
            for key,value in \
              config.languages[language_code]['xml_attributes'].items()
          },
          'text': string
        })
    ...
    traverse_strings(
      container_obj,
      container_name,
      update_strings_callback
    )

  traverse_structure(
    structure,
    update_structure_callback
  )
...
def spreadsheet_to_dict(spreadsheet:str):
  ...
  return [
    {
      language:cell for language,cell in zip(
    ...
    for line_variant_name, line_variant in [
      ...
      for position in ['start', 'end']:
        ...
        for language_code in config.languages.keys():
          ...
          if line_variant_name == 'original_line' and language_code != 'en':
            continue

          match = re_search(
            regex_pattern,
            line_variant[language_code]
          )

          if language_code not in stripped_parts[line_variant_name]:
            stripped_parts[line_variant_name][language_code] = defaultdict(dict)

          stripped_parts[line_variant_name][language_code][position] = match


    for language_code, stripped_language_parts in \
      stripped_parts['updated_stripped_line'].items():
        ...
        if match != '':
          response = error_callback(
            ...
            (
              ...
              match,
              position,
              json.dumps(original_line, indent=4, ensure_ascii=False),
              json.dumps(updated_stripped_line, indent=4, ensure_ascii=False)
          ...
          if response == 'current_language':
            ...
            if response == 'current_en':
              stripped_parts['original_line']['en'][position] = \
                stripped_parts['updated_stripped_line']['en'][position]

            if position == 'start':
              updated_line[language_code] = \
                updated_line[language_code][len(match):]
            else:
              updated_line[language_code] = \
                updated_line[language_code][:-1*len(match)]
