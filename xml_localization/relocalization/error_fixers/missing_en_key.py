from xml_localization.relocalization import utils
        ...
        if 'en' not in line or line['en'] == '':
            ...
            for search_line in extracted_strings:
                if key in search_line and search_line[key] == line[key]:
                    ...
                    accept_fix = \
                        config.error_fixers['missing_en_key']=='automatic' or \
                        utils.prompt(
                            'Replacement `%s` was found. Accept?'
                            % search_line['en']
                        )

                    extracted_strings[index]['en'] = search_line['en'] \
                        if accept_fix else \
                        input('Type your variant: ')

                    if accept_fix:
                        print(colored(
                            'FIXED with %s' % extracted_strings[index]['en'],
                            'green'
                        ))
            ...
            if 'en' not in extracted_strings[index]:
                extracted_strings[index]['en'] = input('Type your variant: ')
