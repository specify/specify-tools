...
def xlsx_to_tsv(in_file_name, out_file_name):
	data_xls = pd.read_excel(in_file_name, engine='openpyxl', dtype=str, index_col=None)
	data_xls.to_csv(out_file_name, encoding='utf-8', index=False, sep=column_separator)
...
def inspect_headers(tsv_file_name):
	...
	print('%s: %s' % (tsv_file_name, first_line))
...
def format_out_file_name(file_name):
	return os.path.join(global_base_dir, 'xlsx_to_tsv/%s.tsv' % file_name)
