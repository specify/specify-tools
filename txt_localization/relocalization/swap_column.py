...
if len(sys.argv) < 4:
    raise Exception('Required argument <second_column_index> is missing')
...
for line in lines:
    ...
    if len(line_parts) >= max_column_index:
        line_parts[first_column_index], line_parts[second_column_index] = line_parts[second_column_index], line_parts[first_column_index]
