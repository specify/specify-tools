...
def run(dry_run=False):
    files = glob.glob(os.path.join(specify6_src_dir_location, '*.properties'))
    ...
    for file_name in target_files:
        base_name, language, = file_name[0:file_name.rfind('_')], file_name[file_name.rfind('_')+1:]
    ...
    for base_name in language_files:
        ...
        for language in languages:
            with open('%s/%s_%s.properties' % (specify6_src_dir_location, base_name, language)) as file:
                ...
                if len(lines) != len(file_data):
                    raise Exception('Number of lines is not the same between "%s" (%d) and "%s" (%d)' % (languages[0], len(file_data), language, len(lines)))
            ...
            if line_data[languages[0]] == '':
                ...
                for language in languages:
                    if line_data[language] == '' or line_data[language][0] != '#':
                        raise Exception('%s begins with "#" at line %d, but %s has value' % (languages[0], i, language))
        ...
        if not dry_run:
            ...
            with open(os.path.join(global_base_dir, 'properties_to_tsv/%s.tsv' % base_name), 'w') as file:
